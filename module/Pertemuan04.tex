...
Model-View-Controller (MVC) adalah pola desain yang digunakan dalam pengembangan perangkat lunak untuk memisahkan tanggung jawab utama dalam aplikasi menjadi tiga komponen yang berbeda. Pemisahan ini membantu meningkatkan keterbacaan kode, mempermudah pemeliharaan, serta memungkinkan pengembangan aplikasi dalam skala besar.

JavaFX menyediakan dukungan yang baik untuk implementasi pola MVC melalui penggunaan \textit{FXML}, \textit{Controller}, dan mekanisme \textit{Property Binding}. Dengan pendekatan ini, tampilan aplikasi dapat didefinisikan secara terpisah dari logika program.

Komponen utama dalam MVC terdiri dari:

\begin{itemize}
\item \textbf{Model} – merepresentasikan data serta logika bisnis aplikasi.
\item \textbf{View} – bertanggung jawab menampilkan data kepada pengguna.
\item \textbf{Controller} – menangani interaksi pengguna serta menghubungkan View dengan Model.
\end{itemize}

Pemisahan ini mempermudah pengujian, pemeliharaan, dan pengembangan aplikasi dalam skala besar karena perubahan pada satu komponen tidak secara langsung memengaruhi komponen lainnya.

\subsection{Bootstrap Aplikasi JavaFX}

Sebelum komponen MVC digunakan, aplikasi JavaFX memerlukan sebuah kelas utama yang bertugas memulai aplikasi dan memuat tampilan awal. Kelas ini biasanya mewarisi kelas \texttt{Application}.

\begin{lstlisting}[style=JavaStyle, caption=Kelas utama untuk memulai aplikasi JavaFX]
package org.mvc;
...
Pada kode di atas, method \texttt{start()} merupakan titik awal aplikasi JavaFX. Method ini memuat file \texttt{FXML} yang berisi definisi tampilan aplikasi menggunakan \texttt{FXMLLoader}. Setelah tampilan dimuat, objek \texttt{Scene} dibuat dan ditampilkan pada \texttt{Stage}, yaitu jendela utama aplikasi.

Kelas ini berfungsi sebagai \textit{bootstrap} aplikasi dan bukan bagian langsung dari komponen MVC.

\subsection{Model dalam JavaFX}

Model merepresentasikan data yang digunakan oleh aplikasi. Dalam JavaFX, model biasanya menggunakan \textit{JavaFX Property} agar data dapat dihubungkan langsung dengan komponen UI melalui mekanisme \textit{binding}.
...
Kelas \texttt{Mahasiswa} berfungsi sebagai Model yang menyimpan data mahasiswa. Properti \texttt{nama} dan \texttt{umur} menggunakan tipe \texttt{Property} dari JavaFX sehingga dapat digunakan dalam mekanisme \textit{binding} dengan komponen UI.

Penggunaan \texttt{StringProperty} dan \texttt{IntegerProperty} memungkinkan perubahan data pada Model secara otomatis tercermin pada tampilan jika properti tersebut dihubungkan dengan komponen UI.

\subsection{View dalam JavaFX}

View dalam JavaFX biasanya didefinisikan menggunakan \textit{FXML}. FXML adalah bahasa markup berbasis XML yang digunakan untuk mendeskripsikan struktur antarmuka pengguna secara deklaratif.

\begin{lstlisting}[style=JavaStyle, caption=View menggunakan FXML dalam JavaFX]
...
<GridPane xmlns:fx="http://javafx.com/fxml" hgap="8.0" vgap="8.0"
          fx:controller="org.mvc.controller.MahasiswaController">
    ...
    <Label text="Umur:" GridPane.columnIndex="0" GridPane.rowIndex="1" />
    <TextField fx:id="txtUmur" GridPane.columnIndex="1" GridPane.rowIndex="1" />

    <Button text="Tambah"
            onAction="#tambahMahasiswa"
            GridPane.columnIndex="1"
            GridPane.rowIndex="2" />

    <TableView fx:id="tableMahasiswa"
               prefHeight="240.0"
               prefWidth="360.0"
        ...
        <columns>
            <TableColumn fx:id="colNama" prefWidth="220.0" text="Nama" />
            <TableColumn fx:id="colUmur" prefWidth="120.0" text="Umur" />
        </columns>
...
Dalam kode FXML di atas, komponen antarmuka seperti \texttt{Label}, \texttt{TextField}, \texttt{Button}, dan \texttt{TableView} didefinisikan secara deklaratif. Atribut \texttt{fx:id} digunakan untuk menghubungkan komponen UI dengan field pada controller, sedangkan atribut \texttt{onAction} digunakan untuk menghubungkan aksi tombol dengan method pada controller.

\subsection{Controller dalam JavaFX}

Controller bertanggung jawab menangani interaksi pengguna serta menghubungkan View dengan Model.
...
public class MahasiswaController {
  ...
  private void initialize() {
    ...
    colNama.setCellValueFactory(
        cellData -> cellData.getValue().namaProperty());

    colUmur.setCellValueFactory(
        cellData -> cellData.getValue().umurProperty());
  ...
  private void tambahMahasiswa() {
    ...
    String nama = txtNama.getText() == null ? ""
    ...
    String umurText = txtUmur.getText() == null ? ""
        : txtUmur.getText().trim();
...
Field yang diberi anotasi \texttt{@FXML} akan dihubungkan secara otomatis dengan komponen UI yang memiliki \texttt{fx:id} yang sama pada file FXML.

Method \texttt{initialize()} dipanggil secara otomatis setelah proses pemuatan FXML selesai. Pada method ini dilakukan konfigurasi tabel serta pengisian data awal.

\texttt{ObservableList} digunakan sebagai struktur data yang dapat dipantau oleh komponen UI. Ketika elemen baru ditambahkan ke dalam list, tampilan tabel akan diperbarui secara otomatis.

Method \texttt{tambahMahasiswa()} dipanggil ketika tombol ``Tambah'' ditekan. Method ini membaca input dari pengguna, membuat objek \texttt{Mahasiswa}, lalu menambahkannya ke dalam daftar data yang ditampilkan pada tabel.

\subsection{Pemisahan Logika Bisnis dan Tampilan}

Dalam pengembangan aplikasi JavaFX, sangat penting untuk memisahkan logika bisnis dari tampilan. Pendekatan ini meningkatkan keterbacaan kode serta mempermudah pengembangan aplikasi.

Beberapa prinsip yang digunakan antara lain:

\begin{itemize}
\item Menggunakan FXML untuk mendefinisikan struktur tampilan.
\item Menggunakan Controller untuk menangani interaksi pengguna.
\item Menggunakan Model untuk menyimpan dan mengelola data aplikasi.
\item Menggunakan \textit{binding} atau \textit{ObservableList} untuk menghubungkan data dengan tampilan.
\end{itemize}

Dengan pendekatan ini, tampilan dapat diubah tanpa memengaruhi logika program, dan logika bisnis dapat dikembangkan secara independen dari komponen UI.

\subsection{Dependency Injection dalam JavaFX}

JavaFX secara sederhana telah menyediakan mekanisme \textit{Dependency Injection} melalui penggunaan anotasi \texttt{@FXML}. Komponen UI yang dideklarasikan dalam FXML akan secara otomatis diinjeksikan ke dalam controller berdasarkan nilai \texttt{fx:id}.

Pendekatan ini memberikan beberapa keuntungan:

\begin{itemize}
\item Mengurangi ketergantungan langsung antar komponen.
\item Mempermudah pengujian unit karena komponen dapat diuji secara terpisah.
\item Memungkinkan perubahan implementasi tanpa memodifikasi struktur utama aplikasi.
\end{itemize}

Dengan memanfaatkan pola MVC serta mekanisme injeksi dependensi yang disediakan oleh JavaFX, arsitektur aplikasi menjadi lebih modular, mudah dipelihara, dan lebih mudah dikembangkan di masa depan.
