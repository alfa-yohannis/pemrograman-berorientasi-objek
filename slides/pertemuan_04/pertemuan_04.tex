...
The Model-View-Controller (MVC) design pattern separates application logic into three main components.

\begin{itemize}
\item \textbf{Model} – Represents application data and business logic.
\item \textbf{View} – Displays the user interface, typically defined using FXML.
\item \textbf{Controller} – Handles user interactions and connects the View with the Model.
\end{itemize}

\bigskip

JavaFX supports MVC naturally through:

\begin{itemize}
\item FXML files for UI layout
\item Controller classes for event handling
\item Model classes representing application data
\end{itemize}

\bigskip

This separation improves maintainability, readability, and scalability of applications.

\end{frame}


\begin{frame}[fragile]{Application Bootstrap in JavaFX}
\vspace{20pt}
\begin{columns}[T]

\column{0.55\textwidth}

A JavaFX application begins with a class that extends
\texttt{Application}.  
This class is responsible for loading the initial user interface.


\textbf{Main Steps}

\begin{itemize}
\item Load the UI from an \textbf{FXML} file
\item Create a \textbf{Scene} containing the UI
\item Display the scene inside a \textbf{Stage}
\end{itemize}
...
\begin{itemize}
\item \texttt{FXMLLoader} loads the UI defined in FXML
\item \texttt{Stage} represents the application window
\item \texttt{Scene} contains the UI components
\end{itemize}
...
The \textbf{Model} represents the application's data and domain objects.

In JavaFX, models often use \textbf{Property classes} so UI components can observe data changes.

\textbf{Key Concepts}

\begin{itemize}
\item The class represents the application data structure.
\item JavaFX \textbf{Property Binding} enables automatic UI updates.
\item Changes in the model can be observed by UI controls such as \texttt{TableView}.
\end{itemize}
...
Common JavaFX property types include: \texttt{StringProperty}, \texttt{IntegerProperty}, \texttt{BooleanProperty}

\end{frame}



\begin{frame}[fragile]{Implementing the View using FXML}

\begin{columns}[T]

\column{0.45\textwidth}

In JavaFX, the \textbf{View} is typically defined using \textbf{FXML},  
an XML-based language used to describe the UI layout.

\textbf{Key Elements}

\begin{itemize}
\item \texttt{fx:controller} links the UI with its controller class.
\item \texttt{fx:id} identifies UI components so they can be injected into the controller.
\item \texttt{onAction} connects UI events to controller methods.
\end{itemize}
...
Using FXML allows developers to separate the  
\textbf{UI structure} from the \textbf{application logic}.
...
The \textbf{Controller} manages interactions between the View and the Model.
...
\begin{itemize}
\item Receive user input from UI components
\item Create or update objects in the Model
\item Update observable data used by the View
\end{itemize}
...
\begin{itemize}
\item \texttt{@FXML} injects UI elements defined in the FXML file.
\item Controller methods handle UI events.
\item The \texttt{ObservableList} automatically updates the UI when data changes.
\end{itemize}
...
Separating UI code from business logic is important for building maintainable JavaFX applications.
...
\begin{itemize}
\item Defining UI layouts using \textbf{FXML}.
\item Handling user interactions in \textbf{Controller classes}.
\item Storing application data in \textbf{Model classes}.
\item Using \textbf{Binding} and \textbf{ObservableList} to synchronize UI and data.
\end{itemize}

\bigskip

This design improves modularity, readability, and scalability.
...
\begin{frame}[fragile]{Dependency Injection using FXML}

\begin{columns}[T]

\column{0.6\textwidth}

JavaFX provides a simple form of \textbf{Dependency Injection} through the
\texttt{@FXML} mechanism.

\medskip

When an FXML file is loaded:

\begin{itemize}
\item UI components declared with \texttt{fx:id} are injected into the Controller.
\item Methods referenced by \texttt{onAction} are connected to UI events.
\end{itemize}

\medskip

This mechanism automatically connects the
\textbf{View (FXML)} and the \textbf{Controller}
without manually creating UI objects.
...
Using the \texttt{@FXML} injection mechanism provides several advantages:

\begin{itemize}
\item Reduces direct coupling between UI and controller code.
\item Simplifies event handling.
\item Improves code readability and modularity.
\item Supports the MVC architecture naturally.
\end{itemize}

\bigskip

By combining MVC with FXML-based injection, JavaFX applications become easier to maintain and extend.
...
\begin{frame}[fragile]{\LARGE{Setting Up a JavaFX Project and Application Structure}}
	\vspace{20pt}
	To create a JavaFX application, follow the steps in the Hello World section  
	(Subsection \ref{gradle_javafx}).
